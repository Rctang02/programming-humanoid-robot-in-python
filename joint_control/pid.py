...
class PIDController(object):
    ...
    def control(self, target, sensor):
        ...
        err = target - sensor + self.y

        self.u = self.u1 + (self.Kp + self.Ki * self.dt + self.Kd/ self.dt) * err -(self.Kp + 2 * self.Kd / self.dt) * self.e1 + self.Kd *self.e2 / self.dt
        self.e2 = self.e1
        self.e1 = err
        self.u1 = self.u
        return self.u
